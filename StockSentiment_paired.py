...
if __name__ == '__main__': #only run function when called instead of immediately upon import
    paired_general_search()

###USERNAME SEARCH###
def paired_username_search():
    tweets_list = []
...
###STOCK PRICES###
    ...
    if price_response == 'Y':
        ...
        if date_response == 'Y':
            stock_data = yf.download(keyword_ticker, start = start_date_raw, end = end_date_raw) #creates Pandas dataframe for desired ticker with start and end dates 
